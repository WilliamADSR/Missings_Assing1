...
Therefore, our research question states: what impact do missing values have on an "active" heart rate" model inference? Consequently, the outcome of our model is *active heart rate*. The predictors are *age* and *smoke*. An interaction effect between *bmi* and *sex* is also included.
For the first model we used the observed data, and for the second model we used the true data. When examining Table 2.5 several differences in estimates and p-values \@ref(tab:reg)
