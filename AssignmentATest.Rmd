...
Thirldy we start by inspecting the missing values. We try to find out where the missing values occur. In \@ref(fig:misspat) we start by giving a global overview of the missings. Then in \@ref(fig:mnar-plot) we compare the distributions for the observed data and the missing values. 
...
When examining Table \@ref(tab:reg) *Regression analysis of True and Observed data* we observe several differences in the beta coefficients, standard error and p-values. Regarding the beta coefficients, missing data caused the beta's of the true data to be systematically underestimated. However, the standard error is systematically overestimated. Furthermore, the missing data caused several predictors to be wrongly non significant (type II error) in the observed data. This goes for *sexfemale* and the *bmi:sexfemale* interaction. Concluding, the missing data causes the standard errors to be greater, resulting in less accurate beta coefficients. Moreover, some p-values turn out non signifianct, caused by underestimated beta coefficients. Thus, the model based on observed data leads to inaccurate inferences.
...
```{r}
fit1 <- glm(mWeight ~ height + age + smoke + intensity + bmi + active + sex,  data = data2, family = 'binomial')
summary(fit1)

fit2 <- glm(mHeight ~ age + smoke + intensity + bmi + active + sex,  data = data2, family = 'binomial')
summary(fit2)

fit3 <- glm(mBmi ~ age + smoke + intensity + height + active + sex,  data = data2, family = 'binomial')
summary(fit3)

fit4 <- glm(mActive ~ age + smoke + intensity + bmi + height + sex,  data = data2, family = 'binomial')
summary(fit4)

fit5 <- glm(mSmoke ~ age  + intensity + bmi + height + sex,  data = data2, family = 'binomial')
summary(fit5)


fit6 <- glm(mSmoke ~ sex, data = data2, family = 'binomial')
fit7 <- glm(mSmoke ~ height, data = data2, family = 'binomial')
fit8 <- glm(mSmoke ~ intensity, data = data2, family = 'binomial')
summary(fit6)
summary(fit7)
summary(fit8)
