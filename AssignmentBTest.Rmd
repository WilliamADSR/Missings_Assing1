...
An imputation is a replacement value for a missing value. A new data set is created for each imputation. There will be 5 imputed data sets by default. 

The method for imputations depends on the measurement level of the target variable. The variables active, height, bmi and weight will be imputed with predictive mean matching. This means that the missing values of these variables will be predicted based on observed values (donors) closest to the missing value in the same column. Then, one of these donors is selected randomly, and its value is used to replace the missing value.

The variable smoke will be imputed using a logistic regression. **How does logistic regression work?** 

The iterations **Say here something about the imputations and iterations (what do they do and why do we use them?)**   
