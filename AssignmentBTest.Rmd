...
```{r convergence, fig.show='hold', fig.cap="Trace plots of the imputations with default options", out.width = '50%', fig.align='center'}
plot(imp)
```
...
```{r density, fig.cap="Density plots of the imputed values with default options", out.width="50%", fig.align='center'}
densityplot(imp)
```

The strip plot and boxplot for active, as shown in Figure \@ref(fig:stripplot1) and Figure \@ref(fig:boxplot1), further increases the validity of the imputation, as the imputed values show a wide variability. For height, and weight, the strip plots show almost no imputations on the lower and higher end. For bmi, the plots show almost no imputations on the higher end. This, however, is in line with the observed data. Based on the strip plots 
alone, the imputations seem sensible.



```{r stripplot1, fig.cap="Strip plots of the imputed values with default options", out.width="50%", fig.align='center'}
strip1 <- stripplot(imp)
strip1
```

```{r boxplot1, fig.cap="Boxplots of the imputed values with default options", out.width="50%", fig.align='center'}
bwplot(imp)
```
...
In \@ref(tab:missInfo1) we present information about the measures of missing values in the imputed data. *RIV* accords for the relative increase in variance, *lambda* is the proportion of the variation in the parameter caused by missing data and *FIV* is basically the same as lambda but FIV also adjusts for the number of imputed datasets. ([https://bookdown.org/mwheymans/bookmi/measures-of-missing-data-information.html])

In \@ref(tab:missInfo1) we observe that the predictor *bmi* stands out from the rest. Furthermore, the missing data for *bmi* accounts for more than 50% of the variation in the parameter. This might has to do with bmi being a derived variable. Later in this document we will try to fix this problem.
...
**Write here something about the passive imputation**
...
As can be seen in the trace plots in Figure \@ref(fig:convergence1), the imputations, represented as lines, intermingle better than the default imputations shown in \@ref(fig:convergence). The improved convergence suggests that there is more information for a solution.

```{r convergence1, out.width = "50%", fig.align='center', fig.cap="Trace plots of the imputations with Passive Imputation"}
plot(imp1)
```

The density plots in Figure \@ref(fig:density1) show the variability of the imputed values, in line with the observed values. There appear no impossible values in these plots.
...
```{r stripplot2, out.width="50%", fig.align='center', fig.cap="Stripplots of the imputations wiht Passive Imputation"}
stripplot(imp1)
```

```{r boxplot2, out.width="50%", fig.align='center', fig.cap="Boxplots of the imputations with Passive Imputation"}
bwplot(imp1)
```
...
kbl(tabImp2, caption = "Regression analysis of the Observed, Imputed and True data",
    booktabs = T,
    col.names = c("b", "SE", "p",
                 "b", "SE", "p",
                 "b", "SE", "p",
  ...
  add_header_above(c("", "Observed Data (N=155)" = 3, "Imputed Data 1(N=306)" = 3, "Imputed Data 2(N=306)"=3, "True Data (N=306)" = 3)) %>% 
  kable_classic(full_width = F, html_font = "Cambria") %>% 
...
kbl(tabMiss2, caption = "Missing information in Imputed Data",
  ...
  add_header_above(c("", "Imputed Data1" = 3, "Imputed Data 2" = 3)) %>% 
    kable_classic(full_width = F, html_font = "Cambria") 
```
