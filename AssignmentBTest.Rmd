...
Next, we perform t-tests on the variables containing missing values to check the type of missingness, either MNAR, MAR, or MCAR. We also provide plots to visualize where the missing values occur. After examination of the missingness and the potential relation between missingness values and other variables, we try to solve the missingness problem with multiple imputation. This will be done using the R-package mice. At first, we use mice’s default settings and examine the results by visualizing the imputed values to check if the imputations are valid. Depending on the outcome, the imputation model will be redefined in order to create plausible imputations. When the outcomes are plausible, we use the data to try and answer the research question: *What influence does a person's age, smoking habits, sex and bmi have on their heart rate during exercise?*
...
fitImp2 <- with(imp2, lm(active ~ age + bmi*sex + smoke))
est2 <- pool(fitImp2)

fitTest <- summary(est2)
fitTest
est2
```
